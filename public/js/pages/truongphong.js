...
function setupDataSyncListeners() {
    ...
    DataSync.on('attendance:created', loadEmployeeAttendance);
    DataSync.on('attendance:updated', loadEmployeeAttendance);
    DataSync.on('attendance:deleted', loadEmployeeAttendance);

    DataSync.on('leave:created', loadEmployeeLeave);
    DataSync.on('leave:updated', loadEmployeeLeave);
    DataSync.on('leave:deleted', loadEmployeeLeave);
    ...
    DataSync.on('account:updated', loadProfileData);
    ...
    // Refresh dashboard if anything relevant changes
    ...
    dashboardEvents.forEach(event => {
        DataSync.on(event, () => {
            ...
            updateDashboardStats();
            // Tạm thời reload local data nếu cần
...
function updateDashboardStats() {
    ...
    if (remainingLeaveCount || remainingLeave) {
        ...
        const available = Math.max(0, 12 - usedDays);
...
function toggleEditProfileForm() {
    ...
    if (formContainer.style.display === 'none') {
        ...
        if (toggleBtn) toggleBtn.innerHTML = '<i class="fas fa-times"></i> Đóng';
    } else {
        formContainer.style.display = 'none';
        if (toggleBtn) toggleBtn.innerHTML = '<i class="fas fa-edit"></i> Chỉnh sửa';
    }
}
...
async function saveProfileChanges() {
    ...
    try {
        ...
        if (document.getElementById('editProfileFormContainer')?.style.display === 'block') {
...
function toggleUploadDocumentForm() {
    ...
    const toggleBtn = document.getElementById('toggleUploadDocBtn');
    ...
    if (formContainer.style.display === 'none') {
        ...
        formContainer.style.display = 'none';
        if (toggleBtn) toggleBtn.innerHTML = '<i class="fas fa-upload"></i> Tải tài liệu';
...
async function saveDocument() {
    ...
    try {
        ...
        if (document.getElementById('uploadDocumentFormContainer')?.style.display === 'block') {
...
function toggleCheckInForm() {
    ...
    if (formContainer.style.display === 'none') {
        ...
        formContainer.style.display = 'block';
        if (btnText) btnText.textContent = 'Đóng form chấm công';
    } else {
        formContainer.style.display = 'none';
        if (btnText) btnText.textContent = 'Chấm công ngay';
    }
}
//lưu
async function submitCheckIn() {
    ...
    try {
        ...
        renderEmployeeAttendance();
        updateDashboardStats();
        if (document.getElementById('checkInFormContainer')?.style.display === 'block') {
            toggleCheckInForm();
        }
    } catch (error) {
...
function toggleLeaveForm() {
    ...
    const btnText = document.getElementById('toggleLeaveBtnText');
    ...
    if (formContainer.style.display === 'none') {
        ...
        formContainer.style.display = 'block';
        if (btnText) btnText.textContent = 'Đóng form';
        if (toggleBtn) toggleBtn.querySelector('i').className = 'fas fa-times';
    } else {
        formContainer.style.display = 'none';
        if (btnText) btnText.textContent = 'Gửi đơn nghỉ phép mới';
        if (toggleBtn) toggleBtn.querySelector('i').className = 'fas fa-plus';
    }
}
...
async function submitLeave() {
    ...
    try {
        ...
        if (document.getElementById('leaveFormContainer')?.style.display === 'block') {
            toggleLeaveForm();
        }
    } catch (error) {
